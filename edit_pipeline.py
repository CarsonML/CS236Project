...
class EditingPipeline(BasePipeline):
    def __call__(
        ...
        image_rec_np = self.decode_latents(latents.detach())
        if only_sample:
            return image_rec
        ...
        #load paramaters to control the mask and wether or not the modified loss function is used
        with open("current_mask.pkl", 'rb') as file:
            mask = pickle.load(file)
        with open("lamb.pkl", 'rb') as file:
            lamb = pickle.load(file)

        # END CS 236 EDITED CODE
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                cur_im = self.decode_latents(latents.detach())
                
                #create mixed image
                updated_im = (1-mask) * cur_im + mask * image_rec_np

                #encode mixed image
                comparison_latents = self.encode_image(updated_im, device)

                # add distance between encoded mixed image and encoded current image to the loss
                loss += torch.mean(torch.sum(torch.square(x_in.chunk(2)[0] - comparison_latents))) * lamb * ((i-1) / 50)
                # END CS 236 EDITED CODE
